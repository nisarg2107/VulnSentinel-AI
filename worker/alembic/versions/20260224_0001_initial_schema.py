"""Create initial VulnSentinel schema."""
...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX IF NOT EXISTS scans_asset_id_idx ON scans (asset_id);")
    op.execute("CREATE INDEX IF NOT EXISTS scans_status_idx ON scans (status);")
    op.execute("CREATE INDEX IF NOT EXISTS scans_created_at_idx ON scans (created_at DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS scan_results_scan_id_idx ON scan_results (scan_id);")
    op.execute("CREATE INDEX IF NOT EXISTS scan_results_vuln_id_idx ON scan_results (vuln_id);")
    op.execute("CREATE INDEX IF NOT EXISTS scan_results_effective_severity_idx ON scan_results (effective_severity);")
    ...
    op.execute("DROP TRIGGER IF EXISTS scans_set_asset_last_scanned_at ON scans;")
    op.execute(
        """
...
def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS scans_set_asset_last_scanned_at ON scans;")
    op.execute("DROP FUNCTION IF EXISTS set_asset_last_scanned_at;")
